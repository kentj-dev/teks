  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="space-y-1 rounded-2xl border border-gray-400 bg-white/50 p-2 shadow-sm dark:bg-white/[.035]">
          {details.map(([label, value]) => (
            <div
              key={label}
              className="group flex items-start gap-4 rounded-xl px-3 py-2.5 hover:bg-black/[.025] dark:hover:bg-white/[.035]"
            >
              <span className="w-20 shrink-0 text-xs text-black/40 dark:text-white/35">
                {label}
              </span>
              <span className="min-w-0 flex-1 break-all text-right text-xs font-medium">
                {value}
              </span>
              {value !== "—" && (
                <button
        ...
        <section className="mt-6">
          <h3 className="mb-2 text-xs font-medium text-[#242424] dark:text-white/40">
            Message
          </h3>
          <div className="rounded-2xl bg-accent p-4 text-sm leading-6 text-white">
            {message.message}
          </div>
        </section>

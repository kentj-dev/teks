...
export function ConversationSidebar({
  ...
  const conversations = useMemo<Conversation[]>(() => {
    ...
    for (const message of messages) {
      if (
        !`${message.to} ${message.from ?? ""} ${message.message}`
          .toLowerCase()
  ...
  return (
    <aside
      className={`${selectedRecipient || messages.length === 0 ? "hidden lg:flex" : "flex"} min-h-0 min-w-0 flex-col border-r border-gray-300 bg-[#fafafa] dark:bg-[#19191b]`}
    >
      <header className="px-4 pb-3 pt-5">
        ...
        <label className="flex h-9 items-center gap-2 rounded-lg border border-gray-400 bg-white px-3 text-black/40 ring-accent/20 focus-within:ring-2 dark:bg-white/[.04] dark:text-white/40">
      ...
      <div className="scrollbar-none flex-1 overflow-y-auto px-2 pb-2">
        {loading && (
          <p className="px-4 py-10 text-center text-sm text-black/40 dark:text-white/35">
            Loading inbox…
          </p>
        )}
        ...
        {conversations.map((conversation) => {
          ...
          return (
            <button
              ...
              className={`group mb-2 w-full rounded-lg border border-gray-400 px-3 py-3 text-left shadow-sm ${selected ? "bg-accent text-white" : "hover:bg-black/[.04] dark:hover:bg-white/[.05]"}`}
            >
              <div className="flex items-baseline justify-between gap-3">
                <span className="truncate text-[15px] font-semibold">
                  {formatPhone(conversation.recipient)}
                </span>
                <span

...
const benefits: Array<[string, string, IconName]> = [
  ...
  ['Mimic provider APIs', 'Point your existing integration at Teks and keep using familiar provider endpoints.', 'developer'],
    ...
    <div className="mx-auto max-w-2xl">
      <div className="text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-black/55 dark:text-white/45">
          Teks lets you test SMS-provider integrations locally without sending real messages or spending SMS credits.
          It mimics supported provider APIs, captures each request, and shows the resulting messages in a local inbox.
        </p>
      </div>
      <div className="mt-9 grid gap-3 sm:grid-cols-3">
        {benefits.map(([title, description, icon]) => (
          <article key={title} className="rounded-2xl border border-gray-400 bg-white p-4 shadow-sm dark:bg-[#19191b]">
      ...
      <div className="mt-4 flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/[.06] p-4 text-left">
        ...
        <p className="text-xs leading-5 text-black/55 dark:text-white/45">
          Teks aims to reproduce each provider&apos;s behavior, but compatibility may not be 100% exact. Provider support
          will continue to improve as new updates are released.
        </p>
      </div>
